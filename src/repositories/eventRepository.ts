...
export class EventRepository {
  ...
  async incrementAvailableTickets(params: { id: string; quantity: number }): Promise<void> {
  ...
  async decrementAvailableTickets(params: { id: string; quantity: number }): Promise<boolean> {
    const { id, quantity } = params;
    try {
      await dynamoDb.send(
        new UpdateCommand({
