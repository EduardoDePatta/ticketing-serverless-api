...
export function encodeRefreshToken(params: { id: string; secret: string }): string {
  return `${params.id}${SEPARATOR}${params.secret}`;
}

export function decodeRefreshToken(params: { token: string }): DecodeRefreshTokenResult {
...
export function hashRefreshTokenSecret(params: { secret: string }): string {
