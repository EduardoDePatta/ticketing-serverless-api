...
export class PasswordHasher {
  ...
  async hash(params: { password: string }): Promise<string> {
  ...
  private async applyPepper(params: { password: string }): Promise<string> {
    const pepper = await this.secretsProvider.getPepper();
    return createHmac("sha256", pepper).update(params.password).digest("base64url");
  }
}
