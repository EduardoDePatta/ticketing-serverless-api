...
export class AwsSecretsProvider implements SecretsProvider {
    ...
    private async fetchSecret(params: { secretId: string }): Promise<string> {
        const result = await this.client.send(
            new GetSecretValueCommand({ SecretId: params.secretId })
        );
        const value = result.SecretString;
        if (typeof value !== "string" || value.length === 0) {
            throw new Error(
                `Secret ${params.secretId} has no usable SecretString`
            );
        }
        return value;
    }
