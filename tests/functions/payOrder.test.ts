    ...
    it("replays cached response without calling the service when idempotency record exists", async () => {
        mockTryReserve.mockResolvedValue({
            ...
            record: {
                pk: "any",
                status: "completed",
                requestHash: "*", // forced match below
                statusCode: 200,
        ...
        // Helper compares hashes; here hashes won't match (we mocked existing.requestHash="*"),
        // so the helper must return 422. This guards against silent replay collisions.
